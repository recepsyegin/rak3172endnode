...
static void SendTxData(void)
{
  /* USER CODE BEGIN SendTxData_1 */
	...
	/*
	const char helloWorld[] = "RecepSYEGIN";

	LmHandlerAppData_t AppData = {
	    .Buffer = (uint8_t *)helloWorld,
	    .Port = LORAWAN_USER_APP_PORT,
	    .BufferSize = strlen(helloWorld)
	};

	if( LmHandlerSend(&AppData, false, false) == LORAMAC_HANDLER_SUCCESS )
	{
	    APP_LOG(TS_OFF, VLEVEL_M, "App: Sent 'Hello World'\r\n");
	...
	*/
	...
	DateTime_t now = Get_SystemTime_UTC();

	    // Zamanı 7 baytlık bir buffer içine paketle
	    uint8_t buffer[7];
	    buffer[0] = (now.year >> 8) & 0xFF;
	    buffer[1] = now.year & 0xFF;
	    buffer[2] = now.month;
	    buffer[3] = now.day;
	    buffer[4] = now.hour;
	    buffer[5] = now.minute;
	    ...
	    // 3. LoRaWAN veri paketi oluştur
	    LmHandlerAppData_t AppData =
	    {
	        .Buffer = buffer,
	        .Port = LORAWAN_USER_APP_PORT, // örneğin 1
